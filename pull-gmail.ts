...
function parseArgs(): CliArgs {
  ...
  for (const arg of Deno.args) {
    ...
    } else if (arg === "--include-to") {
      args.includeTo = true;
    } else if (arg.startsWith("--query=")) {
      args.query = arg.split("=")[1];
    }
  }
...
function buildEmailContent(
  ...
  return `[Email from ${from}${toPart} | Subject: ${subject} | Date: ${date}]\n\n${emailBody}`;
}
...
async function main() {
  ...
  for (const ref of messageRefs) {
    ...
    const content = buildEmailContent(
      ...
      email.subject,
      email.date,
      args.includeTo ? email.to : undefined,
    );
